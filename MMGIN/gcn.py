...
class MultiTaskModel(nn.Module):
    def __init__(self, num_classes_multi, num_classes_binary):
        ...
        if self.fp_type == 'mixed':
            ...
            self.fp_dim = 1024
        #
        self.fc1 = nn.Linear(self.fp_dim, self.fp_1_dim)
        self.act_func = nn.ReLU()
        self.fc2 = nn.Linear(self.fp_1_dim, self.fp_3_dim)
    ...
    def forward(self, data):
        ...
        # print('fp_list_shape',fp_list.shape)
...
def save_result(result_type, result):
    ...
    if result_type == 'all':
        ...
        all_.to_csv('./results_all_multi_gcn.csv')
    else:
        each = pd.DataFrame(result)
        each.to_csv('./results_each_multi_gcn.csv', index=False)
