...
def val():
    ...
    for i,batch_data in enumerate(train_loader):
        ...
        RF_multi = RandomForestClassifier(n_estimators=60)
        RF_multi.fit(fp_list_train, label_multi_train)

        RF_bin = RandomForestClassifier(n_estimators=60)
        RF_bin.fit(fp_list_train, label_bin_train)
    ...
    pre_score_multi = RF_multi.predict_proba(fp_list_test)
    ...
    pred_bin = RF_bin.predict(fp_list_test)
...
def save_result(result_type, result):
    ...
    if result_type == 'all':
        ...
        all_.to_csv('./results_all_multi_RF.csv')
    else:
        each = pd.DataFrame(result)
        each.to_csv('./results_each_multi_RF.csv', index=False)
