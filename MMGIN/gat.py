...
class MultiTaskModel(nn.Module):
    def __init__(self, num_classes_multi, num_classes_binary):
        ...
        self.conv3 = GATConv(in_channels=1024, out_channels=256, heads=8, concat=False)
...
def save_result(result_type, result):
    ...
    if result_type == 'all':
        ...
        all_.to_csv('./results_all_multi_gat.csv')
    else:
        each = pd.DataFrame(result)
        each.to_csv('./results_each_multi_gat.csv', index=False)
